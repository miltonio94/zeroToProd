...
static TRACING: Lazy<()> = Lazy::new(|| {
    let defaul_filter_level = "info".to_string();
    let subscriber_name = "test".to_string();
    if std::env::var("TEST_LOG").is_ok() {
        let subscriber = get_subscriber(subscriber_name, defaul_filter_level, std::io::stdout);
        init_subscriber(subscriber);
    } else {
        let subscriber = get_subscriber(subscriber_name, defaul_filter_level, std::io::sink);
        init_subscriber(subscriber);
    }
});
...
pub async fn configure_database(config: &DatabaseSettings) -> PgPool {
    ...
    connection
        ...
        .await
        .expect("Filed to create database");
